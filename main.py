...
def get_top_seasons(n=10, rebuild_model=False, verbose=False):
    ...
    X = df[['wpa', 'xCompPer', 'xCompletion', 'xYards', 'att', 'season', 'touchdowns', 'interceptions']]
    xEPA_df = model.predict(X) 
    df['xEPA'] = xEPA_df.tolist()   
    df['EPAOE'] = df['epa'] - df['xEPA']
    df = df[['passer_player_id', 'passer_player_name', 'season', 'epa', 'qb_epa', 'wpa', 'xCompletion', 'xYards', 'att', 'touchdowns', 'interceptions', 'xCompPer', 'xEPA', 'MoV', 'EPAOE']].groupby(by=['passer_player_id', 'passer_player_name', 'season']).sum().reset_index()
...
def get_top_scores_by_week(season=2021, week=1, n=32, min=10, rebuild_model=False, verbose=False):
    if not exists('model.txt') or rebuild_model:
        model = build_model(verbose=verbose)
    else:
        model = xgb.XGBRegressor()
...
def top_scores_by_week(season=2021, week=17, n=32):
    min = 10
    leaderboard = get_top_scores_by_week(season, week, n, min)
    players = nfl.import_rosters(years=[season])
    leaderboard = leaderboard.merge(right=players[['player_id', 'season', 'headshot_url']], how='left', left_on=['passer_player_id', 'season'], right_on=['player_id', 'season'])
    build_top_scores_by_week_leaderboard(leaderboard, season, week, min)
    return leaderboard
...
if __name__ == '__main__':
    ...
    update_cache()
    #top_seasons(n=25)
    top_scores_by_week(season=2022, week=8)
    top_scores_by_season(season=2022, week=8)
    scores_as_of_week(week=8, season=2022)
